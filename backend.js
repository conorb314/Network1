...
io.on('connection', (socket) => {
  ...
  socket.on('keydown', ({ keycode, sequenceNumber }) => {
    ...
    if (playerSides.right > 2048)
      backEndPlayers[socket.id].x = 2048 - backEndPlayer.radius

    if (playerSides.top < 0) backEndPlayers[socket.id].y = backEndPlayer.radius

    if (playerSides.bottom > 1152)
      backEndPlayers[socket.id].y = 1152 - backEndPlayer.radius
  })
})
